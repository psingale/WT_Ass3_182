let link = "https://davids-restaurant.herokuapp.com/menu_items.json";
let data= null;

$("document").ready(function(){
    $.get(link,function(jsonObj,success){
        data = jsonObj.menu_items;
        console.log(data.length);
        console.log(jsonObj.menu_items[0])
        createoptions();
       
        let dropdown = document.getElementById("dish");
...
const selectElement = document.querySelector('.dish');

selectElement.addEventListener('change', (event) => {
 index_value=dropdown.selectedIndex-1; showdetails(index_value); console.log(index_value);
...
request.onerror = function() {
  console.error('An error occurred fetching the JSON from ' + link);
};
